...
function write_n_logs(){
    for ((i=1; i<=LOG_COUNT; i++)); do
        ...
        JSON_DATA="{\"request_id\": \"$(uuidgen)\", \"event_type\": \"inventory_event\", \"store_id\": $STORE_ID, \"cust_id\": $CUST_ID, \"category\": \"Camera\", \"sku\": $SKU, \"price\": $PRICE, \"qty\": $QTY, \"discount\": $DISCOUNT, \"gift_wrap\": $GIFT_WRAP, \"variant\": \"MystiqueAutomatoon\", \"priority_shipping\": $PRIORITY_SHIPPING, \"TimeGenerated\": \"$(date +'%Y-%m-%dT%H:%M:%S')\", \"contact_me\": \"$CONTACT_ME\" }"
